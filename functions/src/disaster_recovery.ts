...
export const adminRestoreOperationalMode = onCall<{
  ...
  if (targetMode === 'NORMAL' && (currentMode === 'FINANCIAL_FROZEN' || currentMode === 'EMERGENCY_HALT')) {
    if (recoveryRequestId && typeof recoveryRequestId === 'string') {
      ...
      let txnResult: { requestedBy: string };
      try {
        txnResult = await db.runTransaction(async (transaction) => {
        ...
        if (err.message?.includes('has expired')) {
          await requestDocRef.update({ status: 'EXPIRED' }).catch(() => {});
        }
        throw err;
      }
